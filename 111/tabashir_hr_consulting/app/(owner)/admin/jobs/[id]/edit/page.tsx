...
export default function AdminEditJobPage() {
    ...
    const loadJob = async () => {
      try {
        ...
        if (result.success && result.job) {
          ...
          // If description/requirements are JSON, extract the plain text
          if (typeof description === 'string' && description.startsWith('{')) {
            try {
              const parsedDesc = JSON.parse(description)
              description = extractTextFromJSON(parsedDesc)
            } catch (e) {
          ...
          if (typeof requirements === 'string' && requirements.startsWith('{')) {
            try {
              const parsedReq = JSON.parse(requirements)
              requirements = extractTextFromJSON(parsedReq)
            } catch (e) {
              // If parsing fails, use as is
            }
          }
  ...
  const onSubmit = async (data: JobFormData) => {
    try {
      ...
      const result = await updateAdminJob(jobId, data)
      ...
      if (result.success) {
        ...
        toast.error("Error updating job", {
          description: result.error || "There was an error updating the job posting. Please try again.",
          className: "bg-red-500 text-white",
        })
      }
    } catch (error) {
